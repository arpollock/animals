...
def read_image():
    ...
    print("the height of the color scale image is {} px".format(color_scale_height))
    # for i in range(0, color_scale_height): # print debug make sure works
    #     print(f'{i}: {color_elv_image[i, 0]}')

# TODO: make so is actually accurate https://www.accuware.com/support/convert-coordinates-latitude-and-longitude-into-pixels-xy-of-a-floor-plan-image/
def convert_lat_to_px(latitude=0) -> int:
    if latitude <= UPPER_LEFT_LAT and latitude >= LOWER_RIGHT_LAT:
        percent = (UPPER_LEFT_LAT - latitude) / (UPPER_LEFT_LAT - LOWER_RIGHT_LAT)
...
def convert_long_to_px(longitude=0) -> int:
    if longitude >= UPPER_LEFT_LONG and longitude <= LOWER_RIGHT_LONG:
        percent = abs((UPPER_LEFT_LONG - longitude) / (UPPER_LEFT_LONG - LOWER_RIGHT_LONG))
...
def closet_color_elv(b=0, g=0, r=0) -> float:
    ...
    for i in range(0, color_scale_height): # print debug make sure works
       ...
       color_diff = (abs(r - r_c)**2 + abs(g - g_c)**2 + abs(b - b_c)**2)**(1/2)
       if color_diff < closest_color_dif:
           closest_color_dif = color_diff
           closest_color = i
    # print(f'closest color row: {closest_color}')
    return (MAX_ELEVATION - ((closest_color / color_scale_height) * MAX_ELEVATION))
